...
{
    public class Repairs : Remedy
    {
        public Repairs()
        {
            Name = "Repairs";
            Type = CardType.Remedy.Repairs;
            Score = 0;
        }
        ...
        public override bool CanPlayCard(List<ICard> cardsInPlay)
        {
            ...
            if (canPlayBase)
            {
                if (!cardsInPlay.Any(c => c.GetCardType() == CardType.Hazard.Accident.ToString()))
                {
                    return false;
                }
            }
        ...
        public override void RemoveAffectedCardsFromPlay(List<ICard> cardsInPlay)
        {
            ...
            if (cardsInPlay.Any(c => c.GetCardType() == CardType.Hazard.Accident.ToString()))
            {
                var card = cardsInPlay.FirstOrDefault(c => c.GetCardType() == CardType.Hazard.Accident.ToString());
